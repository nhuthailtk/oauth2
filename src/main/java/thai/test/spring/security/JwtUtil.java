...
public class JwtUtil {
    ...
    private static final String SECRET = "nhuthailamotnguoicuckydethuongvavuitinhdeptraidegan";
    @Autowired
    ...
    public String generateRefreshToken(UserPrincipal user) {
        ...
        try {
            JWTClaimsSet.Builder builder = new JWTClaimsSet.Builder();
            builder.claim(USER_NAME, user.getUsername());
    ...
    public String getUserIdFromRefreshToken(String reToken) throws Exception {

        RefreshToken token = tokenService.findByReToken(reToken);
        if(token.getTokenExpDate().after(new Date())){
            String userId = null;
            try {
                JWTClaimsSet claims = getClaimsFromToken(reToken);
                if (claims != null && isTokenExpired(claims)) {
                    userId= (String) claims.getClaim(USER_NAME);
                }
            } catch (Exception e) {
                logger.error(e.getMessage());
            }
